...
class RestaurantApp {
    ...
    // Gestion des stocks
    async loadStock() {
        // Utiliser les données du menu pour afficher les stocks
    ...
    createStockCard(item) {
        ...
        card.innerHTML = `
            <div class="menu-item-header">
                <div class="menu-item-title">${item.name}</div>
                <div class="stock-info ${stockClass}">${item.stock_quantity} unités</div>
            </div>
            <div class="menu-item-description">${item.description || ''}</div>
            <div class="menu-item-actions">
                <div class="stock-controls">
                    <input type="number" id="stock-input-${item.id}" min="0" value="${item.stock_quantity}" class="stock-input">
                    <button class="btn btn-primary btn-small" onclick="app.updateStockFromInput(${item.id})">
                        Mettre à jour
                    </button>
                </div>
                <div class="quick-actions">
                    <button class="btn btn-success btn-small" onclick="app.adjustStock(${item.id}, 10)" title="Ajouter 10">
                        +10
                    </button>
                    <button class="btn btn-warning btn-small" onclick="app.adjustStock(${item.id}, -10)" title="Retirer 10">
                        -10
                    </button>
                    <button class="btn btn-secondary btn-small" onclick="app.setStock(${item.id})" title="Définir précisément">
                        Définir
                    </button>
                </div>
            </div>
