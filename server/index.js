...
app.get('/api/debug/environment', (req, res) => {
  ...
  const envInfo = {
    ...
    environment: {
      ...
      DB_HOST: process.env.DB_HOST || '[NON DÉFINIE]',
      DB_USER: process.env.DB_USER || '[NON DÉFINIE]',
      DB_NAME: process.env.DB_NAME || '[NON DÉFINIE]',
      DB_PORT: process.env.DB_PORT || '[NON DÉFINIE]',
...
app.post('/api/debug-restaurant', requireAuth, async (req, res) => {
  try {
    ...
    // MySQL forcé dans db-manager.js
...
app.get('/api/debug/tables-structure', requireAuth, async (req, res) => {
  try {
    ...
    const tablesStructure = await query(`
      SELECT column_name, data_type
      FROM information_schema.columns
      WHERE table_name = 'tables' AND table_schema = DATABASE()
    `);
...
app.post('/api/create-restaurant', requireAuth, [
  ...
  try {
    ...
    // MySQL - utiliser insertId pour récupérer l'ID
    const restaurantResult = await run(
      'INSERT INTO restaurants (name, owner_name, email, password_hash, phone, address) VALUES (?, ?, ?, ?, ?, ?)',
      [
        name,
    ...
    console.log('Base de données: MySQL (forcée)');
    ...
    } else if (error.code === 'ER_DUP_ENTRY' && error.message && error.message.includes('email')) {
...
app.listen(PORT, () => {
  ...
  console.log(`🚀 SERVEUR VERSION 2.3 - CONVERSION MYSQL DEPLOYEE`);
});
